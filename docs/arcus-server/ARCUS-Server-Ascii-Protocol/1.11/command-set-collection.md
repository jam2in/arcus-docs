...
- create \< attributes \> - set collection 없을 시에 set 생성 요청. [Item Attribute 설명](arcus-item-attribute.md)을 참조 바란다.
...
- noreply or pipe - 명시하면, response string을 전달받지 않는다. pipe 사용은 [Command Pipelining](/doc/command-pipelining.md)을 참조 바란다.
- \< data \> - 삭제할 데이터 (최대 4KB)
...
- pipe - 명시하면, response string을 전달받지 않는다. [Command Pipelining](scommand-pipelining.md)을 참조 바란다.
