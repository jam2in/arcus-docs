...
B+tree item 구조와 기본 특징은 **[Arcus Server Ascii Protocol 문서의 내용](/arcus-server/ARCUS-Server-Ascii-Protocol/1.11-(latest)/6.-B+tree-Collection-Command)**을
먼저 참고하기 바란다.

## 6-1. BKey(B+Tree Key)와 EFlag(Element Flag)
...
## 6-15. B+tree Element Sort-Merge 조회
...
앞서 설명한 [B+Tree Query 구조체](6.-btree-API.md#6-4-btree-query) 참고하기 바란다.
